...
Это MLOps-проект для предсказания оттока клиентов телеком-компании. Проект охватывает полный жизненный цикл модели: от экспериментов до деплоя.
...
|--------|-----------|------------|
| **Trainer** | Python, scikit-learn, MLflow | Обучает модели, логирует эксперименты, регистрирует лучшую |
| **MLflow Server** | MLflow, SQLite | Хранит эксперименты и модели, управляет версиями |
| **API** | FastAPI, Uvicorn | Отдаёт предсказания, проверяет здоровье, перезагружает модель |
...
Планы по улучшению:
- CI/CD через GitHub Actions (автоматические тесты при пуше)
- Мониторинг (Prometheus + Grafana)
- Unit тесты для API и trainer